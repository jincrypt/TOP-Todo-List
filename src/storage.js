...
function storageCheck() {
  if (storageAvailable("localStorage")) {
    if (localStorage.getItem("storedProjects")) {
      let retrievedProjects = JSON.parse(
...
function defaultList() {
  ...
  homeProject.addTask(
    new Task("Add message when trying to add project without filling form")
  );
  homeProject.addTask(
    new Task("More task details (details, completion date, completion status")
  );
  homeProject.addTask(
    new Task("if task is late and not complete;  turn date red")
  );
